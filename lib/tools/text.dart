...
class MaiMaiTextTool extends PicassoTool {
  ...
  Future<(String, TextStyle)> requestText(BuildContext context, [String? initialText]) async {
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: Builder(builder: (context) {
                        var widgets = [
                          _buildColorNodes(context, style, (p0) => setState(() {
                            style = p0;
                          }), mq.orientation),
                          _buildFontFeatureNodes(context, style, (p0) => setState(() {
                            style = p0;
                          }), mq.orientation)
                        ];
  ...
  Widget _buildColorNodes(BuildContext context, TextStyle style, Function(TextStyle) setStyle, Orientation orientation) {
    return Row(
      mainAxisAlignment: orientation == Orientation.landscape ? MainAxisAlignment.start : MainAxisAlignment.center,
      children: [
        IconButton(onPressed: () {
          setStyle(style.copyWith(color: Colors.white, shadows: TextToolUtils.getBorder(width: 3, blur: 3)));
        ...
        IconButton(onPressed: () {
          setStyle(style.copyWith(color: Colors.red, shadows: TextToolUtils.getBorder(width: 3, blur: 3)));
        }, icon: Container(width: 24, height: 24, decoration: BoxDecoration(
            color: Colors.red, borderRadius: BorderRadius.circular(90)
        ),)),
        IconButton(onPressed: () {
          setStyle(style.copyWith(color: Colors.green, shadows: TextToolUtils.getBorder(width: 3, blur: 3)));
        }, icon: Container(width: 24, height: 24, decoration: BoxDecoration(
            color: Colors.green, borderRadius: BorderRadius.circular(90)
        ),)),
        IconButton(onPressed: () {
          setStyle(style.copyWith(color: Colors.blue, shadows: TextToolUtils.getBorder(width: 3, blur: 3)));
        }, icon: Container(width: 24, height: 24, decoration: BoxDecoration(
            color: Colors.blue, borderRadius: BorderRadius.circular(90)
        ),)),
        IconButton(onPressed: () {
          setStyle(style.copyWith(color: Colors.yellow, shadows: TextToolUtils.getBorder(width: 3, blur: 3)));
        }, icon: Container(width: 24, height: 24, decoration: BoxDecoration(
        ...
        IconButton(onPressed: () {
          ...
          showDialog(
            ...
            builder: (context) => AlertDialog(
              title: const Text('Pick a text color', style: TextStyle(color: Colors.white),),
                ...
                child: Theme(
                  ...
                  child: ColorPicker(
                    ...
                    onColorChanged: (color) {
                      pickerStyle = pickerStyle.copyWith(color: color, shadows: color.computeLuminance() >= 0.3 ? TextToolUtils.getBorder(width: 3, blur: 3) : []);
                    },
                  ),
                ),
              ),
  ...
  Widget _buildFontFeatureNodes(BuildContext context, TextStyle style, Function(TextStyle) setStyle, Orientation orientation) {
    return Row(
      mainAxisAlignment: orientation == Orientation.landscape ? MainAxisAlignment.start : MainAxisAlignment.center,
      children: [
        ChoiceChip(onSelected: (val) {
          setStyle(style.copyWith(fontWeight: val ? FontWeight.bold : FontWeight.normal));
        }, label: const Text("bold", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white),), selected: style.fontWeight == FontWeight.bold,),
        const SizedBox(width: 8,),
        ChoiceChip(onSelected: (val) {
          setStyle(style.copyWith(fontStyle: val ? FontStyle.italic : FontStyle.normal));
        }, label: const Text("italic", style: TextStyle(fontStyle: FontStyle.italic, color: Colors.white),), selected: style.fontStyle == FontStyle.italic,),
        const SizedBox(width: 8,),
        ChoiceChip(onSelected: (val) {
          setStyle(style.copyWith(shadows: val ? TextToolUtils.getBorder(width: 3, blur: 3) : []));
        }, label: const Text("shadows", style: TextStyle(color: Colors.white, ),), selected: style.shadows?.isNotEmpty ?? false,),
      ],
    );
  }
